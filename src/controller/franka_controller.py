...
class FrankaController:
    def __init__(self,
        ...
        if not env:
            env = RealFrankaEnv(step_duration_s=options.step_duration_s,
                                multiplier=options.action_multiplier, action_space=options.action_space)
        self.env = env
        ...
        while amount:
            try:
                ...
                if enable_pre_control:
                    ...
                    print(
                        f"\033[93mPre-control enabled.\033[0m Move the arm into desired recording start position. \nPress \033[92mleft button\033[0m on the space mouse to start or stop recording a new trajectory. ({amount} remaining)")
                    self.enable_spacemouse_control(self.is_pre_controlling, log=False,
                                                   release_gripper_on_exit=False, reset=False)
                else:
                    print(
                        f"Press \033[92mleft button\033[0m on the space mouse to start or stop recording a new trajectory. ({amount} remaining)")
    ...
    def run_from_server(self, save=True, evaluating=True, remaining=None):
        ...
        while True:
            ...
            print(
                f"\033[KPerforming command ({instruction}) for {self.options.max_seconds} seconds. {round(self.options.max_seconds - time_elapsed,2)}s remaining. {remaining} trials remaining.", end="\r")
