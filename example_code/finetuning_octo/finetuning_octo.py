...
def main(_):
    ...
    dataset_train = make_single_dataset(
        dataset_kwargs=dict(
            name="air_net",
    ...
    dataset_val = make_single_dataset(
        dataset_kwargs=dict(
            name="air_net",
            data_dir=FLAGS.data_dir,
            image_obs_keys={"primary": 'primary_image', "wrist": 'wrist_image'},
    ...
    train_data_iter = (
        ...
        .unbatch()
        .shuffle(20000)  # can reduce this if RAM consumption too high
    ...
    model = OctoModel.from_config(
        ...
        text_processor,
        verbose=True,
        dataset_statistics=dataset_train.dataset_statistics,
    )
    ...
    def val_step(state, batch):
        rng, dropout_rng = jax.random.split(state.rng)
        (loss, info), grads = jax.value_and_grad(loss_fn, has_aux=True)(state.model.params, batch, dropout_rng, train=False)
        return info 
